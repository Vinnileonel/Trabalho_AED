...
public class Main {
	...
	public static void embaralhar(Carta bar[]) {
		...
		for (int t = 0; t < 30; t++) { // 30 vezes
			...
			for (int i = 0; i < tamPacote; i++)
				lista.remove(inicio); // retira
			int destino = rnd.nextInt(lista.size() + 1); // insere
			lista.addAll(destino, pacote);
		}
		for (int i = 0; i < bar.length; i++)
			bar[i] = lista.get(i); // copia p/ array
	}
	...
	public static Carta[] retirar_8_9_10(Carta bar[]) {
		...
		// return null(Deixei apanas porque estava no codigo original, mas este trecho
		// não é necessário pois servia apenas para o RUN) ;
	}
	...
	public static void ordenar_jog1(Carta j1[]) {
		// Jogador 1: Bubble Sort
		for (int i = 0; i < j1.length - 1; i++) {
			for (int j = 0; j < j1.length - 1 - i; j++) {
				if (j1[j].compareTo(j1[j + 1]) > 0) {
					Carta temp = j1[j];
					j1[j] = j1[j + 1];
					j1[j + 1] = temp;
				}
